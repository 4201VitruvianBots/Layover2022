...
public class Vision extends SubsystemBase { 
  ...
  public double getTargetHeightAngle() { 
      var corners = limelight.getEntry("tcornxy").getDoubleArray(nullArray); 
      double minPixel = 9999;  
      for(int i = 1; i<corners.length; i+=2){ 
        if(corners[i] < minPixel){ 
          minPixel = corners[i]; 
        } 
      } 
 
      double targetHeightAngle = -Math.atan((minPixel - 120.0) / (240.0 /(2 * Math.tan(Math.toRadians(24.85))))); 
