...
public class Climber extends SubsystemBase { 
  ...
  private double climberHeightUpperLimit = 
      Constants.Climber.climberHeightUpperLimit; // Upper limit of the climber OUTPUT 
  private double climberEncoderUpperLimit = 
      climberHeightUpperLimit 
          * Constants.Climber.encoderUnitsPerRotation 
          * Constants.Climber.climberGearRatio; 
  ...
  public void setElevatorClimberPercentOutput(double value) { 
    ...
    if (value > 0) { 
      if (climberPosition < climberEncoderUpperLimit) { 
        if (Math.abs(climberPosition - climberEncoderUpperLimit) 
          ...
          elevatorClimbMotors[0].set( 
              ...
              Math.min(value, Constants.Climber.maxSpeedLimitsPercent) 
                  * Math.abs(climberPosition - climberEncoderUpperLimit) 
                  / Constants.Climber.climberEncoderSlowdown); 
        } else { 
  ...
  private void updateSmartDashboard() { 
    ...
    SmartDashboard.putNumber("climber height limit", climberHeightUpperLimit); 
  } 
  ...
  public void periodic() { 
    ...
    climberHeightUpperLimit = 
        SmartDashboard.getNumber("climber height limit", Constants.Climber.climberHeightUpperLimit); 
    climberEncoderUpperLimit = 
        climberHeightUpperLimit 
            * Constants.Climber.encoderUnitsPerRotation 
            * Constants.Climber.climberGearRatio; 
    ...
    if ((!climberLowerLimitOverride.get() && elevatorClimbMotors[0].getMotorOutputPercent() < 0) 
        || (getElevatorClimbPosition() >= climberEncoderUpperLimit 
