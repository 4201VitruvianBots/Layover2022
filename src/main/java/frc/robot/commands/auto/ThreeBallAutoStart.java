...
public class ThreeBallAutoStart extends SequentialCommandGroup {
  public ThreeBallAutoStart(
    ...
    PathPlannerTrajectory trajectory1 =
        PathPlanner.loadPath("ThreeBallAutoStart-1", Units.feetToMeters(14), Units.feetToMeters(7), false);

    PathPlannerTrajectory trajectory2 =
        PathPlanner.loadPath("ThreeBallAutoStart-2", Units.feetToMeters(14), Units.feetToMeters(7), false);
    ...
    addCommands(
        ...
        new ParallelDeadlineGroup(new AutoRunIntakeIndexer(intake, indexer).withTimeout(0.7), new SetTurretAbsoluteSetpointDegrees(turret, 10)),

      
        //Intake 1 and Shoot 1
        
        new ParallelDeadlineGroup(new AutoRunIntake(intake, indexer).withTimeout(0.7), new IntakePiston(intake, true)),
        ...
        // Shoot 1,
       new ParallelDeadlineGroup(new AutoRunIndexer(indexer, flywheel, 0.6).withTimeout(0.7), new SetTurretAbsoluteSetpointDegrees(turret, 25)));
  }
}
