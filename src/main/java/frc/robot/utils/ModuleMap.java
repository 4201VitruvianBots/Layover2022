...
/**
 * Contains functions to convert {@link Map}s with {@link ModulePosition} keys to and from arrays so that it's easier to use WPILib swerve
 * functions.
 */
public class ModuleMap {

  /**
   * Creates a {@code Map} with {@link ModulePosition} keys  from multiple values, in the order specified in the {@link
   * ModulePosition} enum.
   *
   ...
   * ModulePosition} enum.
   *
   * <p>You can use this in a for/in loop without needing to supply an empty array like in {@link #orderedValues(Map, Object[]) orderedValues}.
   */
